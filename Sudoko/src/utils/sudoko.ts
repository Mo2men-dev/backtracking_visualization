...
export function solveSudoko(grid: number[][], r: number = 0, c: number = 0, steps: { cell: { r: number, c: number }, grid: number[][] }[], gridSize: number = 3) {
    steps.push({
        cell: { r, c },
    ...
    if (c === gridSize ** 2) {
        return solveSudoko(grid, r + 1, 0, steps)
    }

    if (grid[r][c] !== 0) {
        return solveSudoko(grid, r, c + 1, steps)
    }
    ...
    for (let num = 1; num <= gridSize ** 2; num++) {
        if (isValidSudoko(grid, r, c, num)) {
            ...
            if (solveSudoko(grid, r, c + 1, steps)) {
                return true
            }
